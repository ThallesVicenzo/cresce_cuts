...
import 'package:cresce_cuts/features/input_discount/external/edit_product_data_source_impl.dart';
...
import 'infra/repository/edit_product_repository_impl.dart';
...
class InputDiscountModule extends Module {
  ...
  void binds(Injector i) {
    ...
    i.add<EditProductRepository>(EditProductRepositoryImpl.new);
    i.addInstance<EditProductDataSource>(
      EditProductDataSourceImpl(
        storage: Modular.get(key: 'storage'),
      ),
    );
    i.add<EditProductUseCase>(EditProductUsecaseImpl.new);
  ...
  void routes(RouteManager r) {
    r.child(
      ...
      child: (context) => InputDiscountPage(
        ...
        entity: r.args.data['product'] as ProductEntity?,
